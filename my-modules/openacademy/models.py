...
class Course(models.Model): 
  ...
  def copy(self, default=None):
    ...
    if not copied_count:
      ...
      new_name = u"Copy of {}".format(self.name, copie_count)
...
class Session(models.Model):
  ...
  attendee_ids = fields.Many2many('res.partner', string="Integrantes")
  taken_seats = fields.Float(string = "Asientos ocupados", compute='_take_seats')
  ...
  def _check_instructor_not_in_attendees(self):
    for r in self:
      if r.instructor and r.instructor in r.attendee_ids:
        raise exception.ValidationError("A session's instructor can't be an attendee")
